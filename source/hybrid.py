"""Hybrid quantum-classical reservoir architecture (QRC + ESN).

This module combines a quantum reservoir (QRC) with classical echo state network
(ESN) readout in a sequential hybrid configuration. The QRC preprocesses quantum
input signals, and the ESN applies final regression to compute task performance.
"""
...
class HybridDynamics(QuantumReservoirDynamics, EsnDynamics):
	...
	"""Sequential hybrid QRC+ESN dynamics for quantum input processing.

	This class combines a quantum reservoir layer that preprocesses quantum
	input signals with classical ESN readout dynamics. The two layers are
	coupled through observable-based features extracted from the quantum dynamics.
	"""
	
	def __init__(self, L, J, h, W, dt, N_esn, g, l, task_name, seed=None, n_max_delay=10, ratio_QRC_delay=0.5, **kwargs):
		
		"""Initialize the hybrid quantum-classical architecture.
		...
		J : float
			Quantum reservoir coupling strength.
		h : float
			Quantum transverse field strength.
		W : float
			Quantum disorder strength.
		dt : float
			Quantum evolution time step.
		N_esn : int
			Number of classical reservoir neurons.
		g : float
			Classical ESN feedback gain.
		l : float
			Classical ESN input scaling.
		task_name : str
			Task identifier.
		seed : int or None, optional
			Random seed for reproducibility.
		n_max_delay : int, optional
			Maximum total delay for the hybrid system.
		ratio_QRC_delay : float, optional
			Fraction of total delay assigned to the QRC preprocessing layer.
		**kwargs
			Additional keyword arguments passed to parent classes.

		Notes
		-----
		The QRC processes quantum signals with delay ``n_delay_QRC``, then
		the ESN applies additional delay to reach the target ``n_max_delay``.
		"""
	...
	def serie_hybrid_performance(
		...
		This function trains the QRC and ESN stages to perform quantum-to-classical
		tasks. The QRC first preprocesses quantum observables, then the ESN learns
		from these preprocessed signals. Performance is computed by varying the
		total delay ``n_max_delay`` and optionally the QRC delay fraction.
		...
		l : float
			ESN input scaling.
		Vmp : int, optional
			Time multiplexing nodes for QRC.
		Dmp : int, optional
			Dimension multiplexing for QRC observables.
		task_name : str, optional
			Task identifier.
		N_iter : int, optional
			Number of independent experimental iterations.
		seed : int or None, optional
			Random seed for reproducibility.
		load_obs : bool, optional
			If True, load precomputed QRC observables from disk.
		axis : list[str], optional
			Local observable axes.
		caxis : list[str], optional
			Two-spin correlation axes.
		ccaxis : list[str], optional
			Cross-axis correlation axes.
		n_min_delay : int, optional
			Minimum delay to evaluate.
		n_max_delay : int, optional
			Maximum delay to evaluate.
		ratio_delay_qrc : float, optional
			Fraction of total delay assigned to the QRC stage.
		pm : str, optional
			Performance metric name.
		store : bool, optional
			If True, save results to disk.
		N_rep : int, optional
			Number of ESN repetition steps per timestep.
		qtasks : list[str], optional
			Quantum output tasks to evaluate.
		onlyesn : bool, optional
			If True, also compute standalone ESN performance for comparison.
		N_esn_solely : int, optional
			ESN size when computing standalone performance.
		all_info : bool, optional
			If True, use full quantum tomography for ESN input.
		qrc_prep_perf : bool, optional
			If True, compute and store QRC preprocessing performance separately.
		inp_type : str, optional
			Quantum input type.
		back_action : bool, optional
			If True, include weak measurement back-action effects.
		monitor_axis : str, optional
			Axis along which to monitor the back-action.
		meas_strength : float, optional
			Measurement strength for back-action modeling.
		noise : bool, optional
			If True, add statistical measurement noise to observables.
		N_meas : int, optional
			Number of measurement repetitions for noise modeling.
		reg_prep_param : float, optional
			Ridge regression parameter for QRC preprocessing.
		noise_ridge_corr : bool, optional
			If True, adapt regularization parameter based on noise level.
		**kwargs
			Extra keyword arguments for compatibility.

		Returns
		-------
		tuple
			``(delays, C_mean, C_std)`` containing evaluated delay values and
			mean/standard deviation of performance across iterations.
		"""

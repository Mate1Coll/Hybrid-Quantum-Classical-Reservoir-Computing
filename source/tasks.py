"""Task generation and performance utilities for reservoir computing.

This module defines the `Tasks` class used to create classical and quantum
input signals, compute target outputs, extract quantum features, and
evaluate performance metrics for reservoir computing experiments.
"""
...
class Tasks(BaseSeededClass):
	...
	"""Task generation and evaluation utilities for reservoir computing."""
	...
	def __init__(self, n_max_delay, task_name, n_steps=4000, n_wo=1000, n_train=2000, n_test=1000,
		...
		"""Initialize the task configuration and performance parameters.

		Parameters
		----------
		n_max_delay : int
			Maximum delay of the target task.
		task_name : {'NARMA', 'STM', 'PC', 'Qinp'}
			Type of task to generate.
		n_steps : int, optional
			Total number of timesteps for the experiment.
		n_wo : int, optional
			Warm-up timesteps excluded from training.
		n_train : int, optional
			Training timesteps.
		n_test : int, optional
			Testing timesteps.
		bias : bool, optional
			Whether to include a bias column in the reservoir feature matrix.
		max_bound_input : float or None, optional
			Upper bound for classical input generation (used for NARMA tasks).
		pm : str, optional
			Performance metric name.
		qtasks : list[str], optional
			Quantum output tasks when ``task_name == 'Qinp'``.
		inp_type : str, optional
			Input type for quantum tasks.
		**kwargs
			Extra keyword arguments forwarded to the base seeded class.
		"""
	...
	def get_str_qtasks(self):
		...
		"""Return a concatenated string identifier for the selected quantum tasks.

		This is used for naming output files and directories associated with the
		selected quantum task set.
		"""
		self.strqtasks = ''
	...
	def get_input_signal(self):
		...
		"""Generate the input signal for the configured task.

		Returns
		-------
		numpy.ndarray or list
			A time series of classical or quantum input signals.

		Notes
		-----
		For classical tasks, this produces scalar inputs. For ``Qinp``, this
		produces quantum density matrices according to ``inp_type``.
		"""
	...
	def get_quantum_input_features(self, qtask, sigmas = np.array([sigmax(), sigmay(), sigmaz()])):
		...
		"""Extract quantum features needed to define a quantum target signal.

		Parameters
		----------
		qtask : str
			Quantum task identifier, e.g. ``'Tracerho2'``, ``'Entropy'``, ``'Entanglement'``.
		sigmas : ndarray, optional
			Pauli operators used for spin projection tasks.
		"""
	...
	def get_inputstate_tomography(self, inp_type='qubit'):
		...
		"""Compute tomography-style Pauli expectation values for the quantum input.

		Parameters
		----------
		inp_type : str, optional
			Input type determining the size of the returned observable vector.
		"""
	...
	def reset_quantum_input_features(self):
		...
		"""Reset cached quantum feature arrays to allow fresh computation.

		This clears stored task-specific quantum features between different
		runs or when a new input signal is assigned.
		"""
	...
	def get_output_signal(self, n_delay=None, qtasks=None, reshapeflag=False):
		...
		"""Generate the target output signal for the configured task.

		Parameters
		----------
		n_delay : int or None, optional
			Delay applied to the output signal. If None, uses ``self.n_max_delay``.
		qtasks : list[str] or None, optional
			Subset of quantum tasks to evaluate for ``Qinp`` inputs.
		reshapeflag : bool, optional
			Controls output shape for single quantum tasks.

		Returns
		-------
		ndarray
			Target output time series for the selected task.
		"""
	...
	def split_data(self):
		...
		"""Split reservoir outputs and target signals into train/test sets.

		The method removes the warm-up period and optionally adds a bias
		column to the feature matrix.
	...
	def linear_regression(self, ridge=False):
		...
		"""Train linear readout weights from reservoir states to targets.

		Parameters
		----------
		ridge : bool, optional
			If True, use Ridge regression; otherwise use ordinary least squares.
		...
		if ridge:
			clf = linear_model.Ridge(fit_intercept=False, alpha=1e-3) # we do not intercept value (b=0), then y = Wx.
		else:
	...
	def get_pm(self, y_test, y_pred, debugg=False):
		...
		"""Compute the configured performance metric for predictions.
		...
		y_pred : ndarray
			Predicted values from the readout.
		debugg : bool, optional
			If True, use the training window when reconstructing quantum states.

		Returns
		-------
		float
			Computed performance metric value.
		"""
		...
		if self.pm == 'Capacity':
			if 'Fidelity' in self.qtasks and self.nqtasks == 1:
				C = 0 
				for m in range(y_pred.shape[1]):
					C += np.corrcoef(y_test[:,m], y_pred[:,m], rowvar=False)[0, 1]**2 # Square pearson corr
			else:
	...
	def performance(self, qflag=False, ridge=False):
		...
		"""Evaluate task performance using reservoir readout predictions.

		Parameters
		----------
		qflag : bool, optional
			If True, compute performance separately for each quantum task.
		ridge : bool, optional
			If True, use Ridge regression for readout weights.

		Returns
		-------
		ndarray or float
			Performance values for the evaluated task(s).
		"""
		...
		print(self.pm)
		if 'Fidelity' in self.qtasks and self.pm == 'Fidelity':
			C = 1 - self.C
			C_debugg = 1 - C_debugg
		else:
			C = self.C
